...
module.exports = (ctx, next) => {
	...
	return async (ctx, next) => {
		...
		let isPlayer = /^\/dplayer\//.test(url);         // 播放器目录
		let isUpload = /^\/source\//.test(url);         // 上传文件静态目录
		...
		// player目录
		if(isPlayer){
			let filePath = ctx.request.url.replace(/^\/dplayer\//i, '');
			ctx.request.url = `/dplayer/${config.curPlayerPath}/${filePath}`;
		}

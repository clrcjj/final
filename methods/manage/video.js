...
let DirCoverImgs = async (ctx, next) => {
	...
	await authToken(ctx, next, async () => {
		...
		let promise = dirCatImgs(videoColl, 'source', 'cover', 'videoImage');
		await setResponse(ctx, promise, {
			error: '封面图片列表获取失败',
			success: '封面图片列表获取成功'
		})
...
let DirPosterImgs = async (ctx, next) => {
	...
	await authToken(ctx, next, async () => {
		...
		let promise = dirCatImgs(videoColl, 'source', 'poster', 'poster');
		await setResponse(ctx, promise, {
			error: '海报图片列表获取失败',
			success: '海报图片列表获取成功'
		})
...
let RemoveImages = async (ctx, next) => {
	...
	await authToken(ctx, next, async () => {
		...
		let filePath = path.resolve(__dirname, `../../static/source/${catName}`, fileName);
		...
		if(detill){
			try{
				...
				if(httpResult && httpResult.status === 200){
						...
						// 匹配替换
				   		fse.writeFileSync(curPlayFilePath, httpResult.data.replace(/http:\/\/localhost:11802/gi, remoteUrl));
				   		// 成功
